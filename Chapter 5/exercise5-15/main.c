...
int main(int argc, char *argv[])
{
    ...
    // no params -> my_strcmp
    // direction |         type        | case
    // reverse   || numeric or string || fold_case

    if ((nlines = readlines(lineptr, buf)) > 0) {
        if (fold)
            my_qsort((void**) lineptr, 0, nlines-1, order, (int (*)(void*, void*))strcmp_insensitive);
        else
            my_qsort((void**) lineptr, 0, nlines-1, order, (int (*)(void*, void*))(numeric ? numcmp : my_strcmp));
...
/* low printed first (top), high printed last (bottom) */
int strcmp_insensitive(char *s, char *t) {
    // *s = "APPLE" , *t = "apple"
