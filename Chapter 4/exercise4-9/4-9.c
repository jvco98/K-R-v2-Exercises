/*
	...
	general idea:
	our original getch and ungetch uses a buffer, buf. of type char. Since EOF has an integer value of -1, the buffer will not be able to handle the EOF correctly. so to handle the EOF correctly we will simply change the type of buf to int.
*/
...
int buf[BUFSIZE];
int bufp = 0;
bool pb_flag = false;	// push back flag
char pb_char = '\0';	// pushed back character
